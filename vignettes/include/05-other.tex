...
\proglang{R} contains a handy tool for telling you how big an already allocated 
...
So where is all that extra space coming from?  Simply put, \proglang{R} objects 
...
create a vector, \proglang{R} keeps track of, for example, its length.  If you 
...
\proglang{R} does some aggressive allocation:
\begin{lstlisting}[language=rr]
> object.size(1L:3L)
56 bytes
> object.size(1L:4L)
56 bytes
\end{lstlisting}
Here we see \proglang{R} allocating more bytes than it needs for integer vectors 
...
\proglang{R}.  Once you load \pkg{memuse}, the return for the 
...
imposed on them.  In \proglang{R}, strings by default are allocated to use 56 
