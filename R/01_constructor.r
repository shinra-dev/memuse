...
check.fs <- function(x)
{
  x <- check.names(x)
  if (is.logical(x))
    stop("cannot construct 'fs' object; bad 'unit.names'", call.=F)
  
  x <- check.prefix(x)
  if (is.logical(x))
    stop("cannot construct 'fs' object; bad 'unit.prefix'", call.=F)
  
  x <- check.unit(x)
  if (is.logical(x))
    stop("cannot construct 'fs' object; bad 'unit'", call.=F)
...
### constructor
fs <- function(size=0, unit=.UNIT, unit.prefix=.PREFIX, unit.names=.NAMES)
{
  if (unit == "best")
    u <- "B"
  else
  ...
  # construct
  x <- new("fs", size=size, unit=u, unit.prefix=unit.prefix, unit.names=unit.names)
